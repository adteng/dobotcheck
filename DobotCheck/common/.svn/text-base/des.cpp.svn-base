...
void mydes(unsigned char *boutput, unsigned char *binput,
		   unsigned char *bkey, short int direction)
{
    des_context ctx;
	des_set_key( &ctx,  bkey );
	if(direction == 1)
		des_decrypt( &ctx, binput, boutput );
	else
		des_encrypt( &ctx, binput, boutput );
}

void my3des(unsigned char *boutput, unsigned char *binput,
		   unsigned char *bkey, short int direction)
{
    des3_context ctx3;
	des3_set_2keys( &ctx3, bkey );
	if(direction == 1)
		des3_decrypt( &ctx3, binput, boutput );
	else
		des3_encrypt( &ctx3, binput, boutput );
}
